...
export default function TradePage() {
    ...
    const interval = setInterval(() => {
      setPrice((prev) => {
        ...
        setDigitStats((prevStats) => {
          ...
          newStats[digit] = Math.min(22, newStats[digit] + 1.2);
          return newStats.map((v, i) => (i === digit ? v : Math.max(5, v - 0.25)));
        });
  ...
  useEffect(() => {
    ...
    const lastY = height - ((prices[prices.length - 1] - min) / range) * (height * 0.75) - height * 0.12;
    const markerColor = lastDigit % 2 === 0 ? "#22c55e" : "#ef4444";

    ctx.beginPath();
    ctx.arc(width - 6, lastY, 4, 0, Math.PI * 2);
    ctx.fillStyle = markerColor;
    ctx.fill();

    ctx.fillStyle = markerColor;
    ctx.font = "bold 11px system-ui";
    ctx.fillText(prices[prices.length - 1].toFixed(2), width - 65, lastY - 8);
  }, [prices, lastDigit]);
  ...
  return (
    <div className="min-h-screen bg-[#0b0e17] text-white flex flex-col">
      {/* Header */}
      <header className="h-12 border-b border-white/5 flex items-center justify-between px-3 sticky top-0 bg-[#0b0e17]/95 backdrop-blur z-40">
        <div className="flex items-center gap-2">
          <button
            onClick={() => setMenuOpen(!menuOpen)}
            className="p-1.5 rounded-lg hover:bg-white/5 md:hidden"
          >
          ...
          <Link href="/dashboard" className="flex items-center gap-1.5">
            <div className="w-6 h-6 rounded-md bg-gradient-to-br from-blue-500 to-cyan-400 flex items-center justify-center text-[10px] font-bold">
              TF
            </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <Link
            href="/deposit"
            className="bg-blue-600 hover:bg-blue-500 text-xs px-3 py-1.5 rounded-full transition"
          >
            Deposit
          </Link>
        </div>
      ...
      {/* Mobile Menu */}
      {menuOpen && (
        <div className="md:hidden bg-[#12161f] border-b border-white/5 px-4 py-2 space-y-1 text-sm">
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
        ...
        <div className="flex-1 p-3 flex flex-col min-h-0">
          <div className="flex items-center justify-between mb-2">
            <div>
              ...
              <div className="text-xl font-semibold leading-tight">{price.toFixed(2)}</div>
            </div>
            <div className="text-right">
              <div className="text-xs text-slate-400">Last Digit</div>
              <div className={`text-2xl font-bold leading-tight ${lastDigit % 2 === 0 ? "text-green-400" : "text-red-400"}`}>
                {lastDigit}
              </div>
            </div>
          </div>
          ...
          <div
            ref={containerRef}
            className="flex-1 bg-[#12161f] rounded-xl border border-white/5 overflow-hidden relative min-h-[220px]"
          >
          ...
          <div className="mt-3 grid grid-cols-10 gap-1">
            {digitStats.map((stat, digit) => {
              ...
              let bg = isEven ? "bg-green-900/50" : "bg-red-900/50";
              if (isCurrent) bg = isEven ? "bg-green-500" : "bg-red-500";

              return (
                <div key={digit} className="flex flex-col items-center">
                  <div
                    className={`w-full aspect-square rounded-full flex items-center justify-center text-[11px] font-bold ${bg} ${
                      isCurrent ? "ring-2 ring-white scale-105" : ""
                    } transition-all`}
                  >
                    {digit}
                  </div>
                  <div className="text-[9px] text-slate-500 mt-0.5">{Math.round(stat)}%</div>
                </div>
              );
            })}
          </div>
        ...
        {/* Trading Panel */}
        <div className="w-full lg:w-72 border-t lg:border-t-0 lg:border-l border-white/5 p-3 bg-[#0f1219] overflow-y-auto">
            ...
            <div>
              ...
              <div className="flex items-center gap-1.5">
                <button
                  ...
                  className="w-9 h-9 rounded-lg bg-white/5 hover:bg-white/10 text-lg"
                >
                  −
                </button>
                <input
                  ...
                  className="flex-1 bg-[#1a1f2e] border border-white/10 rounded-lg text-center py-2 text-sm font-medium focus:outline-none focus:border-blue-500"
                />
                <button
